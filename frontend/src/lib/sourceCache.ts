/**
 * NexaFreight — upstream source cache.
 *
    ...
    const inflight = (async () => {
      try {
        ...
        if (entry?.data.length) {
          console.warn(`[NexaFreight] ${key} refresh failed — serving ${entry.data.length} cached cameras`);
        ...
        console.warn(`[NexaFreight] ${key} fetch failed with no cache to fall back on:`, e);

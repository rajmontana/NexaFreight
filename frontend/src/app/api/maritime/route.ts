...
/**
 * NexaFreight — Maritime Intelligence
...
function connectAisStream() {
  ...
  // Map AIS ship types to NexaFreight categories
  const getNexaFreightShipType = (typeCode: number) => {
  ...
  ws.on("message", (data) => {
    try {
      ...
      if (parsed.MessageType === "PositionReport" && parsed.Message?.PositionReport) {
        ...
        existing.type = getNexaFreightShipType(staticData.Type);
      }

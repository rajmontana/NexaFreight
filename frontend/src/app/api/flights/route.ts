...
async function getOpenSkyToken(): Promise<string | null> {
  ...
  try {
    ...
    if (!res.ok) { console.warn('[NexaFreight] OpenSky token failed:', res.status); return null; }
    const data = await res.json();
    if (!data.access_token) {
      console.warn('[NexaFreight] OpenSky token response missing access_token');
      return null;
    }
    ...
    console.warn('[NexaFreight] OpenSky token error:', e);
    return null;
  }
}
...
export async function GET() {
  ...
  fetchPromise = (async () => {
    ...
    if (milRes.status === 'fulfilled') {
      if (milRes.value.ok) {
        try {
          ...
          console.warn('[NexaFreight] adsb.fi mil parse error:', e);
        }
      } else {
        console.warn('[NexaFreight] adsb.fi mil feed returned', milRes.value.status);
    ...
    if (osRes.status === 'fulfilled') {
      if (osRes.value.status === 429) {
        ...
        console.warn('[NexaFreight] OpenSky 429 — cooling down 15 min');
        ...
        try {
          ...
          console.warn('[NexaFreight] OpenSky parse error:', e);
        }
      } else {
        // A rejected token surfaces here as a 401 — previously discarded silently.
        console.warn('[NexaFreight] OpenSky returned', osRes.value.status);
        await osRes.value.body?.cancel();
      }
    }
    ...
    if (!openSkyWorked) {
      source = 'regional';
      console.warn('[NexaFreight] no OpenSky snapshot — falling back to adsb.fi regional sweep');
      ...
      if (allRaw.length === 0) {
        console.error(
          '[NexaFreight] every flight provider returned zero aircraft — ' +
  ...
  try {
    ...
    console.error('[NexaFreight] Flight fetch error:', error);
    fetchPromise = null;
    // Stale-cache fallback: return last known good data instead of blank map
    if (cachedData) {
      console.warn('[NexaFreight] Returning stale flight cache as fallback');

...
/**
 * NexaFreight — Live Conflict Zone Intelligence API
 * 
  ...
  try {
    ...
    const feedPromises = RSS_FEEDS.map(async (url) => {
      try {
        ...
        if (!res.ok) {
          console.log(`[NexaFreight] RSS Fetch Failed for ${url}: ${res.status}`);
          return [];
        }
        
        const xml = await res.text();
        const rawItems = xml.split(/<item>/i).slice(1);
        console.log(`[NexaFreight] RSS ${url} returned ${rawItems.length} items`);
        ...
        console.log(`[NexaFreight] RSS Fetch Error for ${url}:`, (err as Error).message);
        return [];
      }
    });
    ...
    console.log(`[NexaFreight] Mapped ${allEvents.length} conflict events from RSS.`);
  } catch (e) {
...
export async function GET() {
  try {
    ...
    console.error('[NexaFreight] Conflict API error:', error);

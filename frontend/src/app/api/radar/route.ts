...
/**
 * NexaFreight — Internet Outage Detection (IODA)
...
export async function GET() {
  try {
    ...
    const res = await fetch(url, {
      ...
      cache: 'no-store',
      headers: { 'User-Agent': 'NexaFreight/4.2', 'Accept': 'application/json' },
    });

    console.log('[NexaFreight] IODA response status:', res.status);
    ...
    console.error('[NexaFreight] IODA fetch error:', error);
